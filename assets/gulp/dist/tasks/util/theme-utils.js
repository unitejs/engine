...
async function writeIndex(minify, templateName, title, cacheBust, config, headers, scriptIncludes, appLoader, bodyEnd) {
    const join = minify ? "" : "\r\n        ";
...
async function buildIndex(uniteConfig, uniteThemeConfig, buildConfiguration, packageJson) {
    ...
    const substTheme = (line) => line.replace("THEME_COLOR", uniteThemeConfig.themeColor)
    ...
    return writeIndex(buildConfiguration.minify, buildConfiguration.bundle ? "./index-bundle.html" : "./index-no-bundle.html", uniteThemeConfig.title, cacheBust, config, headers, scriptIncludes.map(scriptInclude => `<script src="${scriptInclude}"></script>`), appLoader, bodyEnd);
}
...
exports.buildBrowserConfig = buildBrowserConfig;
async function buildManifestJson(uniteConfig, uniteThemeConfig, packageJson) {
    ...
    for (let i = 0; i < sizes.length; i++) {
        const fname = path.join(uniteConfig.dirs.www.assets, "favicon/", `android-chrome-${sizes[i]}x${sizes[i]}.png`);
...
exports.buildThemeHeaders = buildThemeHeaders;
...
async function buildPwa(uniteConfig, buildConfiguration, packageJson, files, dest, includeRoot) {
    ...
    return asyncUtil.stream(gulp.src(path.join(uniteConfig.dirs.www.build, "assets/pwa/service-worker-template.js"))
