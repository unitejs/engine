...
gulp.task("build-create-pwa", async () => {
    ...
    if (buildConfiguration.pwa) {
        const uniteThemeConfig = await uc.getUniteThemeConfig(uniteConfig);
        const packageJson = await packageConfig.getPackageJson();
        const files = await platformUtils.listFiles(uniteConfig, buildConfiguration);
...
gulp.task("build-post-clean", async () => {
    ...
    try {
        await del(toClean);
        await util.promisify(deleteEmpty)(uniteConfig.dirs.www.dist, {
            verbose: false
        });
    } catch (err) {
        display.error(err);
...
gulp.task("build-html-min", async () => {
    ...
    const buildConfiguration = uc.getBuildConfiguration(uniteConfig, false);
    if (buildConfiguration.minify) {
        return asyncUtil.stream(gulp.src(path.join(uniteConfig.dirs.www.dist, `**/*.${uc.extensionMap(uniteConfig.viewExtensions)}`))
            .pipe(htmlMin({
                collapseWhitespace: true,
                removeComments: true
            }))
...
gulp.task("build-copy-components", async () => {
    const uniteConfig = await uc.getUniteConfig();
    return asyncUtil.stream(gulp.src([path.join(uniteConfig.dirs.www.src, `**/*.${uc.extensionMap(uniteConfig.viewExtensions)}`)])
        .pipe(gulp.dest(uniteConfig.dirs.www.dist)));
});
...
gulp.task("build-css-all", async () => {
    try {
        await util.promisify(runSequence)("build-css-app", "build-css-post-app");
    } catch (err) {
...
gulp.task("build-src-style-all", async () => {
    try {
        await util.promisify(runSequence)("build-css-components", "build-css-post-components");
    } catch (err) {
        display.error("Unhandled error during task", err);
        process.exit(1);
    }
...
gulp.task("build", async () => {
    ...
    try {
        await util.promisify(runSequence)("build-pre", "build-clean", "build-css-components", "build-css-post-components", "build-transpile-modules", "build-transpile", "build-lint", "build-css-app", "build-css-post-app", "build-copy-components", "build-module-config", "build-html-min", "build-bundle-vendor", "build-bundle-app", "build-copy-index", "build-index-min", "build-post-clean", "build-create-pwa");
        if (options.watch) {
