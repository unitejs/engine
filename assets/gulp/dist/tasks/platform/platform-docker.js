...
gulp.task("platform-docker-package", async () => {
    try {
        await util.promisify(runSequence)("platform-docker-clean", "platform-docker-gather", "platform-docker-build-image", "platform-docker-save");
    } catch (err) {
...
gulp.task("platform-docker-gather", async () => {
    ...
    const gatherRoot = path.join("../", uniteConfig.dirs.packagedRoot, `/${packageJson.version}/${platformName.toLowerCase()}/`, options.www);
    ...
    return asyncUtil.stream(gulp.src(path.join(uniteConfig.dirs.www.assetsSrc, `docker/${options.image}/**/*`), {
            dot: true
        })
...
gulp.task("platform-docker-build-image", async () => {
    ...
    const platformRoot = path.resolve(path.join("../", uniteConfig.dirs.packagedRoot, `/${packageJson.version}/`));
    ...
    try {
        const dockerArchive = path.resolve(path.join("../", uniteConfig.dirs.packagedRoot));
        await exec.launch("docker", [
            "save",
