...
import { IUniteImageCliCommand } from "src/types/IUniteImageCliCommand";
import { IUniteThemeConfiguration } from "src/types/IUniteThemeConfiguration";
import * as util from "util";
import * as xml2js from "xml2js";
import { ICordovaConfig } from "../../types/ICordovaConfig";
import { ICordovaConfigImage } from "../../types/ICordovaConfigImage";
import { ICordovaConfigPreference } from "../../types/ICordovaConfigPreference";
...
function loadOptions(uniteConfig: IUniteConfiguration): { platforms: string; save: boolean} {
    ...
    return minimist<{ platforms: string; save: boolean}>(process.argv.slice(2), knownOptions);
}

async function execCordova(args: string[], cwd?: string): Promise<string> {
    const winExt = (/^win/).test(process.platform) ? ".cmd" : "";
    return exec.launch(`cordova${winExt}`, args, cwd);
}

function addPreference(xml: ICordovaConfig, name: string, value: string): void {
    const idx = xml.widget.preference
        .findIndex(pref => pref.$.name === name);
    if (idx < 0) {
        const pref: ICordovaConfigPreference = { $: {} };
        pref.$.name = name;
        pref.$.value = value;
...
function getProjectName(uniteThemeConfig: IUniteThemeConfiguration): string {
    return uniteThemeConfig.title.replace(/ /g, "");
}

function getImageResources(): { [id: string]: { name: string; size?: number; width?: number; height?: number; density?: string}[]} {
    const images: { [id: string]: { name: string; size?: number; width?: number; height?: number; density?: string}[]} = {};
    images.android = [
        // Icons
...
gulp.task("platform-cordova-update-config", async () => {
    try {
        ...
        const xmlOptions: xml2js.OptionsV2 = {};
        const xml = await util.promisify<string, xml2js.OptionsV2, ICordovaConfig>(xml2js.parseString)(cordovaConfigXml.toString(), xmlOptions);
        xml.widget.$.id = "";
...
gulp.task("platform-cordova-create-platforms", async () => {
    try {
        ...
        let currentPlatforms: { [id: string]: string } = {};
        if (fileExists) {
    ...
    for (let i = 0; i < platformsToCreate.length; i++) {
        ...
        if (images[platform]) {
            ...
            for (let j = 0; j < images[platform].length; j++) {
                ...
                const params: IUniteImageCliCommand = {
                    command: "svgToPng",
                ...
                const src = path.join("res", iconOrScreen, platform, img.name)
                    .replace(/\\/g, "/");
                const newImage: ICordovaConfigImage = {};
                if (img.size) {
