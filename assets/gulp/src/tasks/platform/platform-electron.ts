...
function loadOptions(uniteConfig: IUniteConfiguration): { platformArch: string; runtimeVersion: string; save: boolean} {
    ...
    return minimist<{ platformArch: string; runtimeVersion: string; save: boolean}>(process.argv.slice(2), knownOptions);
}
...
gulp.task("platform-electron-clean", async () => {
    ...
    try {
        ...
        await util.promisify<string, deleteEmpty.Options, string[]>(deleteEmpty)(
            path.join(
                "../",
                uniteConfig.dirs.packagedRoot,
                `/${packageJson.version}/`
