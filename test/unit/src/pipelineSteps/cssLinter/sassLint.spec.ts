...
import { SassLint } from "../../../../../src/pipelineSteps/cssLinter/sassLint";
...
describe("SassLint", () => {
    ...
    describe("intitialise", () => {
        ...
        it("can succeed when file does exist", async () => {
            ...
            const res = await obj.initialise(loggerStub, fileSystemMock, uniteConfigurationStub, engineVariablesStub, true);
            Chai.expect(res).to.be.equal(0);
            Chai.expect(engineVariablesStub.getConfiguration<SassLintConfiguration>("SassLint").options["merge-default-rules"]).to.be.equal(false);
        });
        ...
        it("can succeed when file does not exist", async () => {
            ...
            const res = await obj.initialise(loggerStub, fileSystemMock, uniteConfigurationStub, engineVariablesStub, true);
            Chai.expect(res).to.be.equal(0);
            Chai.expect(engineVariablesStub.getConfiguration<SassLintConfiguration>("SassLint").options["merge-default-rules"]).to.be.equal(true);
        });
    });
    ...
    describe("configure", () => {
        ...
        it("can be called with false mainCondition", async () => {
            ...
            const packageJsonDevDependencies: { [id: string]: string } = { "sass-lint": "1.2.3"};
            engineVariablesStub.buildDevDependencies(packageJsonDevDependencies);
    ...
    describe("finalise", () => {
        it("can succeed writing", async () => {
            ...
            const exists = await fileSystemMock.fileExists("./test/unit/temp/www/", ".sasslintrc");
            Chai.expect(exists).to.be.equal(true);
        });

        it("can be called with false mainCondition", async () => {
            await fileSystemMock.directoryCreate("./test/unit/temp/www/");
            await fileSystemMock.fileWriteJson("./test/unit/temp/www/", ".sasslintrc", {});
