...
export class ReadMe extends EnginePipelineStepBase {
    public async process(logger: ILogger, display: IDisplay, fileSystem: IFileSystem, uniteConfiguration: UniteConfiguration, engineVariables: EngineVariables): Promise<number> {
        try {
            ...
            try {
                const exists = await fileSystem.fileExists(engineVariables.rootFolder, "README.md");
                if (exists) {
                    const existingLines = await fileSystem.fileReadLines(engineVariables.rootFolder, "README.md");
                    if (existingLines && existingLines.length > 0) {
                        hasMarker = existingLines[existingLines.length - 1] === marker;
                    }
                }
            } catch (err) {
                super.error(logger, display, "Reading existing README.md failed", err);
                return 0;
            }
            ...
            if (hasMarker) {
                ...
                super.log(logger, display, "Marker has been removed skipping README.md");
            }
